...
def get_topo_order(num_operations, in_degree, neighbors):
    """Sắp xếp Topo nhanh để tính toán Remaining Work."""
...
def calculate_remaining_work(num_operations, neighbors, request_list, topo_order):
    """Tính toán Remaining Work (RW) làm độ ưu tiên gốc ban đầu."""
    min_proc = {i: min(request_list[i].values()) for i in range(num_operations)}
    rw = {i: min_proc[i] for i in range(num_operations)}
    for u in reversed(topo_order):
        max_successor_rw = 0
        for v in neighbors[u]:
            if rw[v] > max_successor_rw:
                max_successor_rw = rw[v]
        rw[u] = min_proc[u] + max_successor_rw
    return rw, min_proc


def find_earliest_gap(busy_intervals, earliest_start, proc_time):
    """Tìm khoảng trống khả thi đầu tiên trên máy (Gap Insertion)."""
    curr_start = earliest_start
    for s, e in busy_intervals:
        if curr_start + proc_time <= s:
            return curr_start
...
def greedy_schedule(num_operations, num_machines, request_list, in_degree, neighbors, predecessors):
    """
    PHIÊN BẢN TINH GIẢN TỐI THƯỢNG: Classic Bottleneck-Loop SWO.
    Giữ nguyên máy móc tự nhiên, dùng vòng lặp để truy vết đường găng và giải tỏa tắc nghẽn.
    """
    # 1. Khởi tạo topo và remaining work làm độ ưu tiên gốc
    topo_order = get_topo_order(num_operations, in_degree, neighbors)
    rw, min_proc = calculate_remaining_work(num_operations, neighbors, request_list, topo_order)
    
    # Độ ưu tiên động của Squeaky Wheel (SWO Priorities)
    priorities = {i: float(rw[i]) for i in range(num_operations)}
    
    best_ub = float('inf')
    best_assignment = None
    best_queue = None
    
    # Chạy 100 vòng lặp truy vết cực nhanh để triệt tiêu mọi điểm nghẽn
    max_iterations = 100 
    
    for iteration in range(max_iterations):
        machine_busy = {m: [] for m in range(num_machines)}
        ...
        # Tập hợp các công việc đã sẵn sàng (không còn ràng buộc phía trước)
        ready_ops = [i for i in range(num_operations) if current_in_degree[i] == 0]
        scheduled_queue = []
        
        # --- BƯỚC 1: LẬP LỊCH THEO ĐỘ ƯU TIÊN SWO ---
        while ready_ops:
            # Chọn công việc có độ ưu tiên (Squeaky Wheel Score) cao nhất hiện tại
            best_op = max(ready_ops, key=lambda x: priorities[x])
            ready_ops.remove(best_op)
            
            # Tính thời điểm sớm nhất có thể bắt đầu dựa trên các công việc đứng trước nó
            earliest_start = 0
            if predecessors[best_op]:
                earliest_start = max(op_completion_time[p] for p in predecessors[best_op])
                
            # Gán máy: Luôn chọn máy hoàn thành sớm nhất thực tế (Không dùng hình phạt ảo)
            best_completion = float('inf')
            best_mach = None
            best_start = None
            
            for machine, proc_time in request_list[best_op].items():
                actual_start = find_earliest_gap(machine_busy[machine], earliest_start, proc_time)
            ...
            # Xác nhận gán việc lên máy
            machine_busy[best_mach].append((best_start, best_completion))
            machine_busy[best_mach].sort()
            
            start_times[best_op] = best_start
            op_completion_time[best_op] = best_completion
            machine_assignment[best_op] = best_mach
            scheduled_queue.append(best_op)
            
            # Cập nhật các công việc tiếp theo đã sẵn sàng
            for neighbor in neighbors[best_op]:
                current_in_degree[neighbor] -= 1
                if current_in_degree[neighbor] == 0:
                    ready_ops.append(neighbor)
                    
        # Tính Makespan (Cận trên UB) của vòng này
        current_ub = max(op_completion_time.values())
        
        # Cập nhật nếu tìm thấy UB tốt hơn toàn cục
        if current_ub < best_ub:
            best_ub = current_ub
            best_assignment = machine_assignment.copy()
            best_queue = scheduled_queue.copy()
            
        # --- BƯỚC 2: TRUY VẾT ĐƯỜNG GĂNG (CRITICAL PATH) ---
        critical_path = set()
        queue_cp = deque([op for op in range(num_operations) if op_completion_time[op] == current_ub])
        
        # Gom nhóm theo máy để tìm liên kết trước/sau trực tiếp trên cùng một máy
        op_by_machine = defaultdict(list)
        for op in range(num_operations):
            m = machine_assignment[op]
            op_by_machine[m].append(op)
            
        machine_prev = {}
        for m in op_by_machine:
            sorted_ops = sorted(op_by_machine[m], key=lambda x: start_times[x])
            for idx in range(1, len(sorted_ops)):
                machine_prev[sorted_ops[idx]] = sorted_ops[idx-1]
                
        visited = set()
        while queue_cp:
            curr = queue_cp.popleft()
            if curr in visited:
                continue
            visited.add(curr)
            critical_path.add(curr)
            
            st = start_times[curr]
            # 1. Ràng buộc công việc: Job trước kết thúc đúng lúc Job sau bắt đầu
            for p in predecessors[curr]:
                if op_completion_time[p] == st:
                    queue_cp.append(p)
            # 2. Ràng buộc máy: Công việc trước trên cùng máy kết thúc đúng lúc công việc này bắt đầu
            if curr in machine_prev:
                p_mach = machine_prev[curr]
                if op_completion_time[p_mach] == st:
                    queue_cp.append(p_mach)
                    
        # --- BƯỚC 3: PHẢN HỒI LỖI (SQUEAKY WHEEL FEEDBACK) ---
        # Tăng mạnh độ ưu tiên cho các công việc nằm trên đường găng (nguyên nhân trực tiếp gây tắc nghẽn)
        # để ở vòng lặp sau chúng được xếp lịch sớm hơn và chiếm các máy trống trước.
        for op in critical_path:
            priorities[op] += min_proc[op] * 1.5 
            
    print(f"Greedy Schedule UB: {best_ub}")
    return best_ub, best_assignment, best_queue
