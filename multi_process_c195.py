...
def get_dynamic_target_4_points(lb, ub, active_targets, worker_index, worker_count=4):
    """Chia 4 mốc đều nhau và né các mốc mà solver khác đang giải"""
    ...
    available_targets = [t for t in ideal_targets if t not in busy_targets]
    
    if not available_targets:
        all_possible = set(range(lb, ub + 1))
        free = list(all_possible - set(busy_targets))
        if free: 
            available_targets = free
        else: 
            return ub
...
def worker_selective_interrupt(
    shared_state, lock, stop_event, interrupt_events, active_targets,
    ...
    while not stop_event.is_set():
        ...
        # 2. KHỞI TẠO HOẶC TÁI SỬ DỤNG SOLVER (ĐỔI SANG CADICAL195)
        ...
        monitor_thread = threading.Thread(
            target=watchdog_event_driven,
            args=(solver, my_event, process_stop_event, name, watchdog_flag)
        )
        ...
        try:
            ...
            # CaDiCaL đôi khi ném C++ Exception khi bị ngắt mạnh
            print(f"[{name}] Lỗi CaDiCaL Solver (thường do interrupt): {e}")
            is_sat = None
        ...
        # ====================================================================
        # ĐẶC TRỊ LỖI CADICAL: NẾU BỊ NGẮT, HỦY DIỆT HOÀN TOÀN SOLVER
        # ====================================================================
        if is_sat is None or was_interrupted:
            print(f"[{name}] Bị ngắt! Tiêu diệt CaDiCaL hiện tại để tránh lỗi memory/UNSAT giả.")
            if solver:
                solver.delete()
                solver = None
            solver_bound = float('inf')
...
def main():
    ...
    with mp.Manager() as manager:
        ...
        processes = [
            mp.Process(target=worker_selective_interrupt, args=args_chung + (0, 4)),
            mp.Process(target=worker_selective_interrupt, args=args_chung + (1, 4)),
            mp.Process(target=worker_selective_interrupt, args=args_chung + (2, 4)),
            mp.Process(target=worker_selective_interrupt, args=args_chung + (3, 4))
        ]

        print("-" * 50)
        print(" 🚀 Start EVENT-DRIVEN search with CaDiCaL195...")
        print("-" * 50)
